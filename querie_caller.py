...
def get_db_alias(db_path):
    ...
    # Remove 'clinvar_' prefix if present
    if filename_no_ext.startswith("clinvar_"):
        return filename_no_ext[8:]  # Remove "clinvar_"
...
def execute_queries(databases: dict):
    ...
    for db_type, db_paths in databases.items():
        ...
        try:
            ...
            # Execute queries
            for i, query_method in enumerate(methods, 1):
                try:
                    print(f"\nQuery {i}:")
...
if __name__ == "__main__":
	if len(sys.argv) < 3:
		print("Usage: python querie_caller.py --ddbb clinvar <clinvar_db> [<clinvar_db>...] --ddbb civic <civic_db> [<civic_db>...]", file=sys.stderr)
		sys.exit(1)
